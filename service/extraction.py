...
# Configure GenAI using the API key
...
def detect_document_type(text: str, model_name: str = "gemini-2.5-pro") -> str:
    """
...
def extract_structured_data_with_gemma(receipt_text: str, model_name: str = "gemini-2.5-pro", debug: bool = False) -> Dict:
...
- invoice_date    (string) : Date format of invoice should be similar as shown 
...
- Please cover all pages of the file 
- Return all the items present in the file"""
...
def extract_receipts_data_with_gemma(receipt_text: str, model_name: str = "gemini-2.5-pro", debug: bool = False) -> Dict:
    if not receipt_text.strip():
        raise ValueError("No receipt text provided for extraction.")
...
def process_invoice_document(pdf_path: str, model_name: str = "gemini-2.5-pro", debug: bool = False) -> Dict:
    """
...
def process_receipt_document(pdf_path: str, model_name: str = "gemini-2.5-pro", debug: bool = False) -> Dict:
    """
    Extracts and processes a document, confirming it's an invoice before extraction.
    Returns structured data as a dictionary or an error dict.
    """
